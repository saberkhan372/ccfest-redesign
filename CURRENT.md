...
- `_config.yml` now has `url: https://ccfest.rocks`. Every page gets a canonical link, Open Graph and Twitter tags; the Register page gets Event JSON-LD built from `_data/event.yml` + `_data/schedule.yml`. No og:image: `assets/ccfest-social.png` is the old wireframe ("Next edition loading"), not Francisca's design.
- Needs others: Francisca — white event facts on orange are 3.31:1 (ink would be 5.36:1). Shristi — mode buttons are `role="tab"` with no tabpanels (should be `aria-pressed` buttons; `animations.js` sets `aria-selected`), and they are 35px tall on phones. Adrienne — enable HTTPS on her GitHub Pages domain, then restore the link. Also a real social-preview image (1200×630) from Francisca.
